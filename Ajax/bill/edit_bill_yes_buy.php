...
if(!token_access_new($token,'yes_bill_edit',$id,"s_form"))
{
...
//проходим по всем материалам в заявке которые содержутся в счете и если у этой заявки материала статус был к оплате и нет других связанных счетов с этим материалом заявкой со статусом равным  12,13 тогда изменяем статус на 11 в работе а статус этой заявки на 9 заказано
$result_score=mysql_time_query($link,'Select b.id,b.id_doc,b.status from z_doc_material_acc as a,z_doc_material as b where a.id_doc_material=b.id and a.id_acc="'.htmlspecialchars(trim($_GET['id'])).'"');
	    ...
	    {
		   mysql_time_query($link,'update z_doc_material set status="11" where id = "'.htmlspecialchars(trim($row_score["id"])).'"');	       
		...
		$result_status=mysql_time_query($link,'SELECT a.* FROM r_status AS a WHERE a.numer_status="11" and a.id_system=13');	
...
$result_score=mysql_time_query($link,'Select b.id,b.id_doc from z_doc_material_acc as a,z_doc_material as b where a.id_doc_material=b.id and a.id_acc="'.htmlspecialchars(trim($_GET['id'])).'" and a.path_buy=1');
		...
		//если все материалы по заявке теперь в оплате то изменить статус заявки на оплата		   
