...
if(token_access_new($token,'pay_implementer',$id,'s_form'))
{
	  ...
	  { 
	     ...
	     { 
	       ...
	       {	
		     ...
		     if(($row_t["summa_debt"]>0)and($row_t["summa_debt"]>=$_GET["summ"]))
		     { 
	   ...
	   {  
         ...
         mysql_time_query($link,'INSERT INTO c_cash (id_cash,numer,date_rco,id_implementer,summa_rco,status,sign_rco) VALUES ("'.$id_user.'","'.$numer.'","'.date("y.m.d").'","'.htmlspecialchars(trim($_GET['id'])).'","'.htmlspecialchars(trim($_GET["summ"])).'","0","0")');
