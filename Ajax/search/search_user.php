...
*/
//**************************************************
 if ((!$role->permission('Наряды','R'))and($sign_admin!=1)and(!$role->permission('Наряды','R')))
{
...
if($query!='')
{
    ...
    for ($i = 0; $i < count($query_mass); $i++) {
        ...
        $lower=plus_string(' and ',$lower,'LOWER(A.name_user) LIKE "%'.$query_mass[$i].'%"');
...
SELECT A.id,A.name_user FROM r_user AS A where '.$lower.'
) 



) Z order by Z.name_user desc limit 0,20';
} else
{
    $sql='Select a.id,a.name_user from r_user as a  ORDER BY a.name_user DESC limit 0,20';
}
...
$num_results_work_zz = $result_work_zz->num_rows;
	    if($num_results_work_zz!=0)
	    {
		   for ($i=0; $i<$num_results_work_zz; $i++)
		   {
			   ...
			   if($query!='')
			   {
			   $query_string.='<li><a style="padding-right: 50px; position: relative;" href="javascript:void(0);" rel="'.$row_work_zz["id"].'">'.search_text_strong(0,$query,$row_work_zz["name_user"]).'<span class="gray-date">('.search_text_strong(0,$query,$row_work_zz["id"]).')</span></a></li>';
			   } else
			   {
			   $query_string.='<li><a style="padding-right: 50px; position: relative;" href="javascript:void(0);" rel="'.$row_work_zz["id"].'">'.$row_work_zz["name_user"].'</a></li>';
			   }
