...
} else
{
	...
	$id=htmlspecialchars($_GET['id']);
        ...
        if(!token_access_new($token,'sign_acc_2021_next',$id,"rema",120)) {
            header404(4, $echo_r);
        }
...
*/
        ...
        if($admin_note!=0)
        {
            ...
            $title=$kto.' согласовал счет №'.$row_list['number'];


            $message=$kto.' согласовал <a class="link-history" href="acc/'.$row_list['id'].'/">Счет №'.$row_list['number'].' от '.date_ex(0,$row_list['date']).'</a>. Счет поступил к - '.$admin_users;
