...
if($error_header!=404){ SEO('docs','','','',$link); } else { SEO('0','','','',$link); }
...
<?
	  ...
	  include_once $url_system.'template/top_docs.php';
...
$sql_mass=$arr_document;
  ...
  {
      $arr_tasks = $edo->my_tasks(3, '=0','ORDER BY d.date_create DESC',limitPage('n_st',$count_write) );
      $sql_mass=$arr_tasks;
  }
  ...
  if(!isset($_GET["tabs"]))
  {
      $arr_document = $edo->my_documents(3, 0 ,false, limitPage('n_st',$count_write));
      $sql_mass=$arr_document;
  }
...
echo'<div class="js-list-docs none"></div>';
  ...
  if(count($sql_mass)==0)
  {
      echo'<div class="help_div da_book1"><div class="not_boolingh"></div><span class="h5"><span>Договоров в данном разделе пока нет.</span></span></div>';
  }
  ...
  if($count_pages>1)
  {
      if(isset($_GET["tabs"]))
      {
          displayPageLink_new('docs/.tabs-'.$_GET["tabs"].'','docs/.tabs-'.$_GET["tabs"].'.page-',"", NumberPageActive('n_st'),$count_pages ,5,9,"journal_oo",1);
      } else
      {
          displayPageLink_new('docs/','docs/.page-',"", NumberPageActive('n_st'),$count_pages ,5,9,"journal_oo",1);
      }
