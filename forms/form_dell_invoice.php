...
$result_t=mysql_time_query($link,'Select a.status,a.number,a.summa from z_invoice as a where a.id="'.htmlspecialchars(trim($_GET['id'])).'"');
...
$token=token_access_compile($_GET['id'],'dell_invoice_xx',$secret);
//составление секретного ключа формы
//составление секретного ключа формы
//составление секретного ключа формы
...
<?
			echo'<h1 class="h111" mor="'.$token.'" for="'.htmlspecialchars(trim($_GET['id'])).'"><span>Удаление накладной</span></h1>';
...
echo'<div class="comme">Вы точно хотите удалить накладную <b>№"'.$row_score["number"].'"</b> на сумму <b>'.$row_score["summa"].'</b> руб.?</div>';
...
?>	
			...
			<br>
 <div class="over">           
<div id="yes_soply_dell_invoice" class="save_button"><i>Удалить</i></div>
