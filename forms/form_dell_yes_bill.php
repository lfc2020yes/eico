...
$token=token_access_compile($_GET['id'],'dell_yes_bill',$secret);
//составление секретного ключа формы
//составление секретного ключа формы
//составление секретного ключа формы
...
<?
			echo'<h1 class="h111" mor="'.$token.'" for="'.htmlspecialchars(trim($_GET['id'])).'"><span>Отменить решение</span></h1>';

		
	$date_graf2  = explode("-",$row_t["date"]);
					  // echo'<span class="stock_name_mat">от '.$date_graf2[2].'.'.$date_graf2[1].'.'.$date_graf2[0].'</span>';		
		
echo'<div class="comme">Вы точно хотите отменить статус <b>к оплате</b> у <b>"Cчета №'.$row_t["number"].'"</b> от '.$date_graf2[2].'.'.$date_graf2[1].'.'.$date_graf2[0].'?</div>';
	...
	?>	
			...
			<br>
 <div class="over">           
<div id="yes_bill_no11" class="save_button"><i>ДА</i></div>
