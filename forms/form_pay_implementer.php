...
<?
			echo'<h1 class="h111" mor="'.$token.'" for="'.htmlspecialchars(trim($_GET['id'])).'"><span>Выдача денежных средств</span></h1>';
...
<input type=hidden name="ref" value="00">
	   ...
	   $result_t1=mysql_time_query($link,'Select a.id,a.summa_debt,a.summa_paid,a.summa_made,a.implementer  from i_implementer as a where a.id="'.htmlspecialchars(trim($id)).'" and a.summa_debt>0');
	   ...
	   {  	
		   ...
		   ?>
			...
			<span class="clock_table"></span>
<?
			echo'<h3 class="head_h pay_uf" mor="'.$token.'" for="'.htmlspecialchars(trim($_GET['id'])).'">Выдача денежных средств<div></div></h3>';
?>	
...
ToolTip();
		    ...
		    echo'<div class="input-width"><div class="width-setter payy"><label style="display: block;">MAX ('.$row_t1["summa_debt"].')</label><input name="number_rr" id="number_rrss" max="'.$row_t1["summa_debt"].'" placeholder="" class="input_pay_imp input_100_x white_inp_x" autocomplete="off" type="text"></div></div>';
	   ...
	   {  	
		   ...
		   echo'<span data-tooltip="Есть неподписанные выдачи" class="yes_pay_grey_e">%</span>';
			...
			<br>
 <div class="over">            
<div id="yes_pay" class="save_button"><i>выдать</i></div>
<div id="no_rd" class="no_button"><i>Отменить</i></div>            
