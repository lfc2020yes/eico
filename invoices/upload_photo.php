...
{
   $result=mysql_time_query($link,'select id from z_invoice_attach_defect where name="'.htmlspecialchars(trim($log)).'"');
...
if ((isset($_POST["id"]))and((is_numeric($_POST["id"])))) 
{
	  ...
	  { 
	     ...
	     { 
		   ...
		   $result_t=mysql_time_query($link,'Select a.* from z_invoice_material as a where a.id="'.htmlspecialchars(trim($_POST['id'])).'"');
	...
	mysql_time_query($link,'INSERT INTO z_invoice_attach_defect (id_invoice_material,name,type,type_invoice,displayOrder,visible) VALUES ("'.htmlspecialchars(trim($_POST["id"])).'","","","1","0","0")');
	$ID_D=mysqli_insert_id($link);		   			 		

$uploaddir = $_SERVER["DOCUMENT_ROOT"].'/invoices/scan_material/';
...
if (move_uploaded_file($_FILES['thefile']['tmp_name'], $uploadfile)) {
 ...
 //загрузился
  mysql_time_query($link,'update z_invoice_attach_defect set visible="1",name="'.$name_imgs.'",type="'.$allowedExts[array_search($extension, $allowedExts)].'" where id = "'.$ID_D.'"');
	
} else
{
	mysqli_query($link,'delete FROM z_invoice_attach_defect where id="'.$ID_D.'"');
