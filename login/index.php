...
//echo(password_crypt_(4,'123','solo'));
...
if ((array_key_exists('ref', $_POST))and(array_key_exists('email', $_POST))and(array_key_exists('password', $_POST)))
{
    ...
    /* проверка поля логин */
    if(trim($_POST['email'])=='') {  $error='1';  } else  { if(log_email(trim($_POST['email']))==0) {  $error='1';   }}
    ...
    if($error=="0")
    {
        ...
        $result_o=mysql_time_query($link,'select id,login from r_user where login="'.htmlspecialchars(trim($_POST['email'])).'"');
...
//echo(password_crypt_($row1222_o["id"],htmlspecialchars(trim($_POST['password'])),$row1222_o["email"]));
            ...
            if($num_results!=0)
            {
                ...
                setcookie("lis",$_POST['email'], time() + 60 * 60 * 24 * 365, "/", $base_cookie, false, true); //на год
                //echo($row["id"]);
            } else
            {
...
<table id="container_login" align="center"><tr><td style="padding-left:0px; padding-right:0px;">
            ...
            <div class="jurnal_bg bg_form login_2022_bg" id="gradient">

                <div class="formi_css_login input-block-2020  bg-login-1">
<div>
                    <h3 class="h3-f">Вход в Личный кабинет</h3>
                    <?
...
/*
                        ...
                        echo '<!--input start-->';
                        echo '<div class="margin-input" style="margin-bottom: 10px;"><div class="input_2021 gray-color"><label><i>Логин</i></label><input id="email_formi" name="email" value="'.$_POST["email"].'" class="input_new_2021 gloab required  no_upperr " style="padding-right: 100px;" autocomplete="off" type="text"><div class="div_new_2021"></div></div></div>';
                        echo '<!--input end	-->';


                        echo '<!--input start-->';
                        echo '<div class="margin-input" style="margin-bottom: 10px;"><div class="input_2021 gray-color"><label><i>Пароль</i><span>*</span></label><input id="password_formi" name="password" value="" class="input_new_2021 gloab required  no_upperr " style="padding-right: 100px;" autocomplete="off" type="password"><div class="div_new_2021"></div></div></div>';
                        echo '<!--input end	-->';
                        /*
...
*/
                    ...
                    } else
                    {
                        if(!isset($_COOKIE["lis"]))
                        {
                            ...
                            echo '<!--input start-->';
                            echo '<div class="margin-input" style="margin-bottom: 10px;"><div class="input_2021 gray-color"><label><i>Логин</i></label><input id="email_formi" name="email"  class="input_new_2021 gloab required  no_upperr " style="padding-right: 100px;" autocomplete="off" type="text"><div class="div_new_2021"></div></div></div>';
                            echo '<!--input end	-->';
                       ...
                       /*     echo'<div class="input-width"><div class="width-setter"><input type="text" value="'.$_COOKIE["lis"].'" name="email" id="email_formi" placeholder="Логин" class="input_f_1" autocomplete="off"></div></div><br>';*/
                            ...
                            echo '<!--input start-->';
                            echo '<div class="margin-input" style="margin-bottom: 10px;"><div class="input_2021 gray-color"><label><i>Логин</i></label><input id="email_formi" name="email" value="'.$_COOKIE["lis"].'" class="input_new_2021 gloab required  no_upperr " style="padding-right: 100px;" autocomplete="off" type="text"><div class="div_new_2021"></div></div></div>';
                            echo '<!--input end	-->';
...
<?
                    ...
                    if(isset($_POST["ref"]))
                    {
                        ...
                        if($error11==3)
                        {
                            echo'<div class="text_formi_error" style="line-height: 16px;">Слишком много неуспешных попыток авторизации. Необходимо подождать '.$jdat.' '.PadejNumber($jdat,'минуту,минуты,минут').'</div>';
                        } else
                        {
                    ...
                    */
                    ...
                    <div style="height:70px;">
                        <div class="login_button_2022" id="yes1" style="float:left;">Далее</div>
                ...
                <div class="bg_2022_j bg-login-2">
                </div>
