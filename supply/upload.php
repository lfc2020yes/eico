...
{
   $result=mysql_time_query($link,'select id from z_acc_attach where name="'.htmlspecialchars(trim($log)).'"');
...
if ((isset($_POST["id"]))and((is_numeric($_POST["id"])))) 
{
	  ...
	  { 
		 ...
		 if (($role->permission('Счета','A'))or($sign_admin==1))
	     { 
	       ...
	       {	
		     ...
		     if((($row_t["status"]==1))and($row_t["id_user"]==$id_user))
		     { 
	...
	mysql_time_query($link,'INSERT INTO z_acc_attach (id_acc,name,type,displayOrder,visible) VALUES ("'.htmlspecialchars(trim($_POST["id"])).'","","","0","0")');
	$ID_D=mysqli_insert_id($link);		   			 		

$uploaddir = $_SERVER["DOCUMENT_ROOT"].'/supply/scan/';
...
if (move_uploaded_file($_FILES['thefile']['tmp_name'], $uploadfile)) {
 ...
 //загрузился
  mysql_time_query($link,'update z_acc_attach set visible="1",name="'.$name_imgs.'",type="'.$allowedExts[array_search($extension, $allowedExts)].'" where id = "'.$ID_D.'"');
	
} else
{
	mysqli_query($link,'delete FROM z_acc_attach where id="'.$ID_D.'"');
