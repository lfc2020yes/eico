...
function FORM_table_XY(&$row_TREE)
{
...
<?
  ...
  //------------------------------выбрать форму  c полем X
  $status=0;
  $sql_FORM='select * from _FORM where PARAGRAF="'.$row_TREE["PARAGRAF"].'" and MASTER in ("X") and VISIBLE=1 order by displayOrder';
  ...
  if ($num_results_FORM>0)
  {  	$row_FORM = mysql_fetch_array($result_FORM);    //Строка описания поля в форме
    ...
    for($i=0; $i<$num_results_X; $i++)
    {
        ...
        $X_SOURCE_DATA[]  =$row_X[$row_FORM["SOURCE_FIELD"]];
    ...
    {   if ($j%2==1) { $bgcolor='bgcolor="#CCFFCC"';  $class='fil1'; }
        ...
        for ($i=0; $i<count($X_SOURCE_DATA); $i++) //---------по X
        {
            ...
            $sql_new=new Tsql ('select * from '.$row_TREE['ID_TABLE']
                             ...
                             .' and '.$X_COLUMN_FIELD[$i].'="'.$X_SOURCE_DATA[$i].'"'.$where_in
                              );
            ////echo_pp(&$row_TREE,$sql_new->sql);      ////
            if ( $sql_new->num=1)
            {  $sql_new->NEXT();
...
function FORM_table_XY_post(&$row_TREE)
{
//--------------------------------------Получить инфу по маске
    ...
    $status=0;
//------------------------------------------------------------------------XXX
    $sql_FX='select * from _FORM where PARAGRAF="'.$row_TREE["PARAGRAF"].'" and MASTER in ("X") and VISIBLE=1 order by displayOrder';
    $result_FX=mysql_query($sql_FX);
    $num_results_FX = mysql_num_rows($result_FX);
    echo_pp(&$row_TREE,$sql_FX);    ///
    if($num_results_FX>0)                            //row_FX  row_FY
    { $row_FX = mysql_fetch_array($result_FX);      //Описание поля Z - может быть теxt bool cbox
    }
    else $status=-1;
//------------------------------------------------------------------------YYY
 if ($status==0)
 {
    ...
    echo_pp(&$row_TREE,$sql_FY);    ///
    if($num_results_FY>0)                            //row_FX  row_FY
    { $row_FY = mysql_fetch_array($result_FY);      //Описание поля Z - может быть теxt bool cbox
    }
    else $status=-2;
 }
//------------------------------------------------------------------------ZZZ
 if ($status==0)
 {
  ...
  {   $row_FD = mysql_fetch_array($result_FD);      //Описание поля Z - может быть теxt bool cbox
   ...
   if ($status==0)
   {
//------------------------------- Если поле по X дате -> удалять промежуток
