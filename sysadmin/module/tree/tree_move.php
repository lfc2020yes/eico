...
function TREE_MOVE(&$row_TREE) {
 ...
 { $sql_MT->NEXT();
     ...
     $t = new tree_m(false,0,$sql_MT->row['PARAGRAF']);
     unset ($t); 
...
<?php
   if ($_POST["do"]==$row_TREE["PARAGRAF"]) { 
       if(($newPARAGRAF<>'') and ($newPARENT<>'')) {
          $t = new tree_m(true,0,$sql_MT->row['PARAGRAF'],$newPARAGRAF,$newPARENT,$newID_TABLE); 
          unset ($t);    
       }
       else echo "<tfoot><tr><td colspan=3>Заполните параметры перемещения</td></tr></tfoot>";
   } 
