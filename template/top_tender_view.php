...
<div class="menu-09  input-line" style="z-index:150;">
    ...
    <div class="menu-09-left">
        <a href="/" class="menu-09-global"></a><a href="tender/" class="menu-09-prev"><i></i></a>

        <?
        //$D = explode('.', $_COOKIE["basket1_".$id_user."_".htmlspecialchars(trim($_GET['id']))]);
        echo'<span class="menu-09-pc-h" ><span class="js-tender-name-top"> Тендер №'.$row_list["id"].' </span >';
/*
        if(count($D)>0)
        {
...
*/
  ...
  <input name="tk_sign" value="'.token_access_compile($_GET['id'],'sign_tender_order',$secret).'" type="hidden">
</form>';
            echo'<div class="save_button pod_zay js-pod_pro add_clients green-bb">Согласовать   →</div><div style="display:none;" class="save_button add_zay js-add-tender-save add_clients yellow-style">Сохранить   →</div>';
...
<div class="choice-radio" data-tooltip="' . $but_mass["name_action"] . ' с замечанием"><div class="center_vert1"><i class=""></i><input name="kto_komy" class="js-type-soft-view1" value="0" type="hidden"></div></div></div></div>
   ...
   <form id="js-form-next-sign" class="none" action="tender/sign_yes/' . $_GET["id"] . '/" style=" padding:0; margin:0;" method="post" enctype="multipart/form-data">
  <input name="tk" value="' . token_access_compile($_GET['id'], 'sign_tender_2021_next', $secret) . '" type="hidden">  <input name="tk1" value="wEVR678vmrIrt" type="hidden">
</form>';
    }
...
*/
                    $echo_bb.='<div class="save_button pod_zay js-pod_pro add_clients red-bb js-reject-tender '.$class_by.'">Отклонить   ⨰</div><div class="save_button pod_zay pod_pro add_clients js-forward-tender '.$class_by.'">Переслать   ⥃</div>';
