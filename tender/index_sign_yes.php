...
} else
{
	...
	$id=htmlspecialchars($_GET['id']);
        ...
        if(!token_access_new($token,'sign_tender_2021_next',$id,"rema",120)) {
            header404(4, $echo_r);
        }
...
*/
        ...
        if($admin_note!=0)
        {
            ...
            $title=$kto.' согласовал тендер №'.$_GET['id'];


            $message=$kto.' согласовал тендер - <a class="link-history" href="tender/' . $_GET['id'] . '/">' . $row_list['name'] . '</a>. Заявка поступила к - '.$admin_users;
...
//отправка уведомления
    ...
    $kto=name_sql_x($id_user);
    $title=$kto.' выполнил тендер №'.$_GET['id'];

    $message=$kto.' Выполнил задачу по тендеру - <a class="link-history" href="tender/' . $_GET['id'] . '/">' . $row_list['name'] . '</a>.';
    notification_send_admin($title,$message,$user_admin,$id_user,$link);
