...
} else
{
	...
	$id=htmlspecialchars($_GET['id']);
        ...
        if(!token_access_new($token,'sign_worder_forward_2',$id,"rema",120)) {
...
$text_not='Вам поступила задача <a class="link-history" href="worder/'.$value['id'].'/">Наряд №'.$value['numer_doc'].'</a> - '.$row_list1["object_name"].' ('.$row_town["town"].', '.$row_town["kvartal"].')'.$val["descriptor_task"];
