...
} else
{
	...
	$id=htmlspecialchars($_GET['id']);
        ...
        if(!token_access_new($token,'sign_worder_reject',$id,"rema",120)) {
            header404(4, $echo_r);
        }
...
//если уже проведен но отклоняется после следующими сотрудниками, то распроводим сначала
if($row_list["status"]==20) {

    $result = nariad_sign($link, $_GET["id"], 0, $sign_level, $id_user);
//nariad_sign(&$mysqli,$id_narid, $sinedd,$sign_level, $id_user
//echo($result);

    if ($result !== true) {
        //echo("!");
        header("Location:" . $base_usr . "/worder/" . htmlspecialchars(trim($_GET['id'])) . '/no/100/');
        die();
    }
}
...
mysql_time_query($link,'update n_nariad set status="8" where id = "'.htmlspecialchars(trim($_GET['id'])).'"');
//mysql_time_query($link,'update n_nariad set id_signed'.$niz_podpis.'="0" where id = "'.htmlspecialchars(trim($_GET['id'])).'"');
mysql_time_query($link,'update n_nariad set id_signed0="0",id_signed1="0",id_signed2="0" where id = "'.htmlspecialchars(trim($_GET['id'])).'"');
/*
