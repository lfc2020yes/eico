...
} else
{
	...
	$id=htmlspecialchars($_GET['id']);
        ...
        if(!token_access_new($token,'sign_worder_remark',$id,"rema",120)) {
            header404(4, $echo_r);
        }
...
/*
                $text_not='Вам поступила задача по <a class="link-history" href="worder/' . $_GET['id'] . '/">Наряду №' . $row_list['numer_doc'] . '</a> -  '.$row_list1["object_name"].' ('.$row_town["town"].', '.$row_town["kvartal"].').' . $value["description"].'Детали в разделе <a class="link-history" href="supply/">cнабжение</a>.';
*/
